...
public class SimpleGame {
	...
	public String convertTime(int seconds){
		...
		if(seconds < 0) {
			...
			output =  Integer.toString(hours) + ":" +  Integer.toString(mins) + ":" +  Integer.toString(secs);
			System.out.println(output);
		}
		return output;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println("Enter 1 to play converting time or Enter 2 to play digits Sum");
		int gameNumber = sc.nextInt();
		
		if(gameNumber == 1) {
			System.out.println("Enter an integer to convert");
		} else if (gameNumber == 2) {
			System.out.println("Enter an for calculate sum of digits");
		}
		
		int gameInteger = sc.nextInt();
		
		
		if(gameNumber == 1) {
			simpleGame.convertTime(gameInteger);
		} else if (gameNumber == 2) {
			simpleGame.digitsSum(gameInteger);
		}
